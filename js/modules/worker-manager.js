...
class FallbackSearchProcessor {
    ...
    simpleSearch(query, filters = {}) {
        ...
        this.searchIndex.songs.forEach(song => {
            ...
            if (filters.genre && song.genre !== filters.genre) passesFilters = false;
            if (filters.artist && song.artist !== filters.artist) passesFilters = false;
            if (filters.album && song.album !== filters.album) passesFilters = false;
