...
document.querySelector('.search-bar').addEventListener('input', (e) => {
  ...
  logger.debug('Search query changed', { query, queryLength: query.length });
  filterSongs(query);
});
...
function filterSongs(query) {
  ...
  logger.info('Song filtering completed', {
    query,
  ...
  if (filtered.length === 0 && songs.length > 0) {
    ...
    logger.debug('No songs matched search query');
  } else {
    logger.debug('Rendering filtered song list', { count: filtered.length });
...
document.getElementById('nav-home').addEventListener('click', () => {
  logger.debug('Navigation: Home button clicked');
  showScreen('home-screen');
});
document.getElementById('nav-playlists').addEventListener('click', () => {
  logger.debug('Navigation: Playlists button clicked');
  showScreen('playlists-screen');
});
document.getElementById('nav-settings').addEventListener('click', () => {
  logger.debug('Navigation: Settings button clicked');
  showScreen('settings-screen');
});
document.getElementById('nav-home2').addEventListener('click', () => {
  logger.debug('Navigation: Home button 2 clicked');
  showScreen('home-screen');
});
document.getElementById('nav-playlists2').addEventListener('click', () => {
  logger.debug('Navigation: Playlists button 2 clicked');
  showScreen('playlists-screen');
});
document.getElementById('nav-settings2').addEventListener('click', () => {
  logger.debug('Navigation: Settings button 2 clicked');
  showScreen('settings-screen');
});
...
document.getElementById('set-theme-ice').addEventListener('click', () => {
  logger.info('Theme changed to ice', { previousTheme: currentTheme });
  document.body.className = 'theme-ice';
  currentTheme = 'ice';
});

document.getElementById('set-theme-amoled').addEventListener('click', () => {
  logger.info('Theme changed to AMOLED', { previousTheme: currentTheme });
...
document.getElementById('clear-data').addEventListener('click', async () => {
  ...
  if (confirm('Are you sure you want to clear all data? This cannot be undone.')) {
    logger.info('User confirmed data clearing - proceeding with deletion');
...
document.getElementById('dark-mode').addEventListener('change', (e) => {
  ...
  logger.info('Dark mode setting changed', { mode });
...
document.getElementById('dark-schedule').addEventListener('change', (e) => {
  ...
  logger.info('Dark mode schedule toggled', { enabled });
