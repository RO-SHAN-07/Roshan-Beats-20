...
function buildSearchIndex(songs) {
  ...
  songs.forEach(song => {
    ...
    if (song.artist) {
      searchIndex.artists.add(song.artist);
    }
    if (song.album) {
      searchIndex.albums.add(song.album);
    }
    if (song.genre) {
      searchIndex.genres.add(song.genre);
    }
  });
...
self.onmessage = function(e) {
  ...
  try {
    ...
    case 'ADD_SONG':
      ...
      // Update filter sets
      if (data.song.artist) {
        searchIndex.artists.add(data.song.artist);
      }
      if (data.song.album) {
        searchIndex.albums.add(data.song.album);
      }
      if (data.song.genre) {
        searchIndex.genres.add(data.song.genre);
      }
