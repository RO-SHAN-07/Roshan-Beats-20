...
async function initMap() {
  ...
  // Try to get user's location with Capacitor Geolocation
      ...
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          (position) => {
            ...
            console.log('Fallback location:', userLocation);
            // Redraw map with location
            drawMapBackground(ctx, canvas, userLocation);
            drawMapElements();
          },
          (error) => {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          console.log('Web location:', userLocation);
          drawMapBackground(ctx, canvas, userLocation);
          drawMapElements();
        },
        (error) => {
...
function drawMapBackground(ctx, canvas, userLocation = null) {
  // Draw simple map background
  ctx.fillStyle = '#e0f7fa';
  ctx.fillRect(0, 0, canvas.width, canvas.height);

  // Draw user's location if available
  if (userLocation) {
    ...
    ctx.fillStyle = '#000';
    ctx.font = '12px Arial';
    ctx.fillText('You', canvas.width / 2 + 12, canvas.height / 2 - 5);
  }
...
function drawMapElements() {
  const canvas = document.getElementById('map-canvas');
  const ctx = canvas.getContext('2d');
  // Draw pins (offset from center if user location is shown)
  mapPins.forEach(pin => {
...
async function startAR() {
  try {
    ...
    // Check if Capacitor Camera is available (Android)
    if (window.Capacitor && window.Capacitor.Plugins && window.Capacitor.Plugins.Camera) {
      // Use Capacitor Camera for Android - but for live stream, still need WebRTC
      // Capacitor Camera is for photos, so fallback to WebRTC for live view
...
async function captureARPhoto() {
  try {
    if (window.Capacitor && window.Capacitor.Plugins && window.Capacitor.Plugins.Camera) {
      ...
      // Process the captured image with AR overlay
      console.log('AR Photo captured:', image);
      alert('AR Photo captured! Check console for details.');
    } else {
      // Fallback: use canvas to capture current frame
      const video = document.getElementById('ar-video');
      const overlay = document.getElementById('ar-overlay');
      ...
      console.log('AR Photo captured (fallback):', dataUrl);
      alert('AR Photo captured! Check console for details.');
    }
  } catch (error) {
    alert('Failed to capture AR photo: ' + error.message);
  }
}
